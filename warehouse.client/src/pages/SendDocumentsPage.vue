<template>
  ...
  <div v-else class="card shadow-sm p-3">
    <div class="row">
      <div class="col-md-8">
        <span class="fs-4">Документы отгрузки</span>
      </div>
      <div class="col-md-4">
        <button @click="router.push('send-documents/add')" type="submit" class="btn btn-success w-100">Добавить</button>
      </div>
    </div>
    ...
    <form class="my-3" @submit.prevent="fetchDocuments()">
      ...
      <div class="row mt-3">
        ...
        <div class="col-md-3">
          <label class="d-inline-block" for="client">Поиск по клиенту:</label>
          <select id="client" v-model="filter.clientIds" class="form-select mt-1" multiple>
            <option v-for="client in availableClients" :key="client.guid" :value="client.guid">
              {{ client.name }}
            </option>
          </select>
        </div>
      ...
      <table class="table table-bordered align-middle">
        ...
        <tbody>
          <tr v-for="document in documents" :key="document.guid">
            <td>
              <button v-if="!document.isSigned" @click="deleteDocument(document.guid)" class="btn btn-danger">x</button>
              <button v-if="!document.isSigned" @click="router.push(`/send-documents/${document.guid}`)" class="btn btn-warning mx-1">...</button>
              <button @click="signDocument(document.guid)" class="btn" :class="document.isSigned ? 'btn-danger' : 'btn-success'">
                {{ document.isSigned ? "Отозвать" : "Подписать" }}
              </button>
            </td>
            ...
            <td>
              <span class="text-white badge" :class="document.isSigned ? 'bg-success' : 'bg-secondary'">
...
<script setup>
  ...
  const fetchDocuments = async () => {
    try {
      documents.value = (await axios.post('/api/send-document/filter', filter.value)).data;
    } catch (ex) {
  ...
  const deleteDocument = async (id) => {
    try {
      await axios.delete(`/api/send-document/${id}`);
  ...
  const signDocument = async (id) => {
    try {
      await axios.post(`/api/send-document/sign/${id}`);
      await fetchDocuments();
    } catch (ex) {
      alert(ex.response?.data || "Произошла ошибка");
    }
  ...
  const initPage = async () => {
    try {
      ...
      availableDocumentNumbers.value = (await axios.get('/api/send-document/numbers')).data;
