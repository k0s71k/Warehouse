  ...
  <div class="card shadow-sm">
    <div class="card-body align-items-center">
      <form class="w-100 py-3" @submit.prevent="addDocument()">
        <span class="fs-3 text-center">Добавление документа отгрузки</span>
        <div class="col">
          ...
          <select id="client" v-model="document.client" class="form-select mt-1">
            <option selected v-for="client in availableClients" :key="client.guid" :value="client">
              {{ client.name }}
            </option>
          </select>
          <table class="table">
...
<script setup>
  ...
  const addDocument = async () => {
    try {
      await axios.post('/api/send-document/add', document.value);
